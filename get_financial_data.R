...
df$vix_5d <- (as.numeric(df[,2]) - lag(as.numeric(df[,1]),5))/as.numeric(df[,1])
df$tnx_5d <- (as.numeric(df[,4]) - lag(as.numeric(df[,3]),5))/as.numeric(df[,3])
write.csv(df,"financial_data.csv")
